...
typedef struct MallocMetadata3 {
    ...
    MallocMetadata3 *next_in_bucket;
    MallocMetadata3 *prev_in_bucket;
} Metadata3;
...
std::string testWild(void *array[MAX_ALLOC]) {
	...
	checkStats(0, 0, __LINE__);
	// printMemory<Metadata3>(memory_start_addr, false);
...
std::string testReallocWildLikePiazza(void *array[MAX_ALLOC]) {
	std::string expected = "" ;
	...
	checkStats(0, 0, __LINE__);
	for (int i = 0 ; i < default_block_size ; ++i) {
		((char *) array[3])[i] = (char) i;
	}
	DO_MALLOC(array[3] = srealloc(array[3], default_block_size *3));
	checkStats(0, 0, __LINE__);
	for (int i = 0 ; i < default_block_size ; ++i) {
...
void checkStats(size_t bytes_mmap, int blocks_mmap, int line_number) {
    ...
    static const bool be_annoying = false;
	if (be_annoying && _num_allocated_bytes() != current_stats.num_allocated_bytes) {
	...
	if (be_annoying && _num_free_blocks() != current_stats.num_free_blocks) {
	...
	if (be_annoying && _num_free_bytes() != current_stats.num_free_bytes) {
...
int main() {
	...
	for (int i = 0 ; i < NUM_FUNC ; ++i) {
        // if(i == 7) __asm__ volatile("int $0x03");
		pid_t pid = fork();
		if (pid == 0) {
